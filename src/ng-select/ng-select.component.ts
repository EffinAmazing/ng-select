...
export const NG_SELECT_DEFAULT_CONFIG = new InjectionToken<NgSelectConfig>(
  'ng-select-default-options'
);
...
export class NgSelectComponent
  ...
  @HostBinding('class.ng-select-opened') get isOpened() {
    return this.isOpen;
  }
  @HostBinding('class.ng-select-disabled') get isDisabled() {
  ...
  get selectedValues() {
    this.showMessage(
      `selectedValues:`,
      this.selectedItems.map(x => x.value)
    );
  ...
  showAddTag() {
    ...
    return (
      this.addTag &&
      this.filterValue &&
      !this.itemsList.items.some(
        x => x.label.toLowerCase() === this.filterValue.toLowerCase()
      ) &&
      !this.loading &&
      !this.readOnly
    );
  ...
  onInputFocus() {
    ...
    (<HTMLElement>this.elementRef.nativeElement).classList.add(
      'ng-select-focused'
    );
  ...
  onInputBlur() {
    ...
    (<HTMLElement>this.elementRef.nativeElement).classList.remove(
      'ng-select-focused'
    );
  ...
  private _updateNgModel() {
    ...
    if (this.multiple) {
      ...
      this.changeEvent.emit(
        this.selectedItems[0] && this.selectedItems[0].value
      );
      this.searchEvent.emit(
        this.selectedItems[0] && this.selectedItems[0].label
      );
    }
  ...
  private _handleEnter($event: KeyboardEvent) {
    ...
    if (this.isOpen) {
      if (this.itemsList.markedItem) {
        if (
          !this.itemsList.markedItem.label &&
          !this.itemsList.markedItem.value['name']
        ) {
          this.close();
        }
        this.toggleItem(this.itemsList.markedItem);
